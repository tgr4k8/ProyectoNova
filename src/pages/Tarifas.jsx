...
const Tarifas = () => {
    ...
    function contratar(plan) {
        // Guardamos el plan en el contexto (persistirá en localStorage)
        setPlan({
            id: plan.id,
            title: plan.titulo,
        ...
        // Abrimos el modal (paso 1) sobre la página actual
            ...
            <div className="tarifas-contenido">
                ...
                <div className="tarjetas-tarifa">
                    {planes.map((plan) => (
                        ...
                        >
                            {plan.destacado && (
                                <div className="destacadoMensaje">El más elegido</div>
                            )}
                            <h5>{plan.titulo}</h5>
                            <p className="precio">{plan.display}</p>
                            <button
                                className={`btn-contratarTarifa ${seleccionado === plan.id ? "destacado" : ""}`}
                                onClick={(e) => { e.stopPropagation(); contratar(plan); }}
                            >
                                Contratar
                            </button>
                        </div>
